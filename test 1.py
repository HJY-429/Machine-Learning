...
def compute_centroids(X_pca_k, L_train):
    ...
    for movement in [0, 1, 2]:
        ...
        centroids.append(np.mean(X_pca_k[:, indices], axis=1))
    return centroids


# Test classifier accuracy for different k values
k_values = range(0, 144)
...
for k in k_values:
    ...
    for i in range(np.shape(X_train)[1]):
        distances = np.linalg.norm(np.dot(X_train_pca.components_[:,:k].T, X_train[:, i]) - centroids, axis=1)
...
plt.figure()
plt.plot(k_values, [accuracies[k] for k in k_values], marker='o', linestyle='-', color='purple')
...
color = ['r', 'g', 'b']
for i in range(3):
    plt.scatter(np.dot(PC1t, X_test[:, i*100:(i+1)*100]), np.dot(PC2t, X_test[:, i*100:(i+1)*100]),
    label=movement[i], c=color[i])
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.title("2D PCA Projection")
...
color = ['r', 'g', 'b']
for i in range(3):
    ax.scatter(np.dot(PC1t, X_test[:, i*100:(i+1)*100]), np.dot(PC2t, X_test[:, i*100:(i+1)*100]),
               np.dot(PC3t, X_test[:, i*100:(i+1)*100]),
    label=movement[i], c=color[i])
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.set_zlabel("PC3")
...
for k in k_values:
    ...
    for i in range(np.shape(X_test)[1]):
        distances = np.linalg.norm(np.dot(X_train_pca.components_[:,:k].T, X_test[:, i]) - centroids, axis=1)
        predicted_label = np.argmin(distances)
...
# Plot accuracy results
plt.figure()
plt.plot(k_values, [accuraciest[k] for k in k_values], marker='o', linestyle='-', color='purple')
plt.xlabel("Number of PCA Modes (k)")
plt.ylabel("Classifier Accuracy")
plt.title("Accuracy vs. Number of PCA Modes")
plt.grid()
