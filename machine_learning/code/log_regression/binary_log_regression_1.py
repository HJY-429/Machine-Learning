...
class BinaryLogReg:
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        n = len(y)
        Jacobian = np.sum(np.log(1/self.mu(X,y))/n) + self._lambda * np.linalg.norm(self.weight, 2)**2
        return Jacobian
        # raise NotImplementedError("Your Code Goes Here")
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        n = len(y)
        gradient_J_weight = ((-y*(1-self.mu(X,y)))@X)/n+2*self._lambda*self.weight
        # print(f'gradient_J_weight = {abs(gradient_J_weight).max()}')  #TEST
        if np.isnan(abs(gradient_J_weight).max()): ##TEST
            error 
        return gradient_J_weight
        # raise NotImplementedError("Your Code Goes Here")
    ...
    def gradient_J_bias(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        n = len(y)
        gradient_J_bias = (-y*(1-self.mu(X,y))).sum()/n
        return gradient_J_bias
        # raise NotImplementedError("Your Code Goes Here")
    ...
    def train(
        ...
        # standardize X and y
        # print("TEST std", np.std(X_train, axis=0), np.std(X_test, axis=0))
        # X_train = (X_train - np.mean(X_train, axis=0)) / np.std(X_train, axis=0)
        # y_train = (y_train - y_train.mean())/np.std(y_train)
        # X_test = (X_test - np.mean(X_test, axis=0)) / np.std(X_test, axis=0)
        # y_test = (y_test - y_test.mean())/np.std(y_test)
        ...
        for epoch in range(epochs):
            ...
            for batch in range(num_batches):
                ...
                # print("TEST", self.weight.max(), self.weight.min())

                self.step(X_batch, y_batch, learning_rate=learning_rate)
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, 
                          test_learning_rate, epochs, batch_size)
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title(f"learning rate = {test_learning_rate}, batch size = {batch_size}")
    plt.legend()
    plt.show()
