...
def plotLearningCurve(errorTrain, errorTest, regLambda, degree):
    ...
    plt.title(f"Learning Curve (d={degree}, lambda={regLambda})", fontdict=dict(size=16))
    plt.xlabel("Training samples", fontdict=dict(size=13))
    plt.ylabel("Error", fontdict=dict(size=13))
    plt.yscale("log")
    plt.ylim(top=maxY)
    plt.xlim((minX, 10))
...
if __name__ == "__main__":
    ...
    plt.suptitle("PolyRegression Learning Curves with Different Params", fontsize=20)
    plt.tight_layout()
    plt.legend(fontsize=15)
    plt.savefig("PolyRegression Learning Curves with Different Params.pdf")
    plt.show()
