...
\author{
    ...
    \normalsize{Due: \textbf{Wednesday} November 5, 2025 11:59pm}\\
    \normalsize{\textbf{A}: 106 points, \textbf{B}: 20 points}
}
\date{{}}
\maketitle
...
You may use common computing packages (such as \texttt{numpy} or \texttt{scipy}), but do not use an existing Lasso solver (e.g., of \texttt{scikit-learn}).
...
\begin{aprob}
    ...
    In order to avoid this, use the Lasso algorithm implemented in the previous problem. 
...
\begin{bprob}
    ...
    \begin{enumerate}
        \item \points{3} Show that $\widehat{\beta}_j \sim \mathcal{N}(\beta_j^*, (X^T X)^{-1}_{j, j})$ for each $j = 1, ..., d$. \emph{(Hint: see \href{https://courses.cs.washington.edu/courses/cse446/23au/assignments/hw2/conf_intervals.pdf}{\textcolor{blue}{notes}} on confidence intervals.)}
