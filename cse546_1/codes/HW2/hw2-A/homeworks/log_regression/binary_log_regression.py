...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        t = y * (self.bias + np.dot(X, self.weight))
        return 1 / (1 + np.exp(-t))
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        t = y * (self.bias + np.dot(X, self.weight))
        log_loss = np.mean(np.log(1 + np.exp(-t)))
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        n = len(y)
        mu = self.mu(X, y)
        grad_w = 1/n * np.sum((mu-1) * y * X.T, axis=1) + 2 * self._lambda * self.weight
        return grad_w
    ...
    def train(
        ...
        for e in range(epochs):
            ...
            for batch in range(num_batches):
                ...
                end = min((batch + 1) * batch_size, n_train)
...
if __name__ == "__main__":
    ...
    print("Part b:\n")
    model_b = BinaryLogReg()
    (x_train, y_train), (x_test, y_test) = load_2_7_mnist()
    
    history_b = model_b.train(x_train, y_train, x_test, y_test, 
                              learning_rate=1e-1, epochs=50, batch_size=len(x_train))

    # Plots b
    plt.plot(history_b["train_losses"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_b["test_losses"], 'go-', label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("loss", fontdict=dict(size=13))
    plt.title("(b) Loss vs Iteration (lr=0.1)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(b)_loss.pdf")
    plt.show()
    
    plt.plot(history_b["train_errors"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_b["test_errors"], 'go-', label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("misclassification error", fontdict=dict(size=13))
    plt.title("(b) Error vs Iteration (lr=0.1)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(b)_errors.pdf")
    plt.show()


    print("\nPart c:\n")
    model_c = BinaryLogReg()
    history_c = model_c.train(x_train, y_train, x_test, y_test,
                             learning_rate=1e-4, epochs=50, batch_size=1)

    # Plots c
    plt.plot(history_c["train_losses"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_c["test_losses"], 'go-', label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("loss", fontdict=dict(size=13))
    plt.title("Loss vs Iteration (SGD, batch_size=1, lr=1e-4)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(c)_loss.pdf")
    plt.show()

    plt.plot(history_c["train_errors"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_c["test_errors"], 'go-', label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("misclassification error", fontdict=dict(size=13))
    plt.title("Error vs Iteration (SGD, batch_size=1, lr=1e-4)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(c)_errors.pdf")
    plt.show()


    print("\nPart d:\n")
    model_d = BinaryLogReg()
    history_d = model_d.train(x_train, y_train, x_test, y_test, epochs=50, batch_size=100)

    # Plots d
    plt.plot(history_d["train_losses"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_d["test_losses"], 'go-',label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("loss", fontdict=dict(size=13))
    plt.title("Loss vs Iteration (SGD, batch_size=100, lr=0.01)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(d)_loss.pdf")
    plt.show()
    
    plt.plot(history_d["train_errors"], 'bo-', label="Train", linewidth=2, markersize=3)
    plt.plot(history_d["test_errors"], 'go-', label="Test", linewidth=2, markersize=3)
    plt.xlabel("epochs", fontdict=dict(size=13))
    plt.ylabel("misclassification error", fontdict=dict(size=13))
    plt.title("Error vs Iteration (SGD, batch_size=100, lr=0.01)", fontdict=dict(size=15))
    plt.legend(fontsize=12)
    plt.savefig("(d)_errors.pdf")
    plt.show()
